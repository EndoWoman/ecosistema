...
export default function Dashboard({accountName}:{accountName:string}) {
  ...
  return <div className="shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Navegación del CEO"><a className="accounts-link" href="/cuentas">Usuarios y permisos</a>{sections.map((s,i)=><button key={s} className={section===s ? "active" : ""} aria-current={section===s ? "page" : undefined} onClick={()=>setSection(s)}><Icon name={icons[i]}/>{s}</button>)}</nav>
    ...
    <div className="workspace">
      <header className="topbar"><div className="demo-switch"><Icon name="shield"/><span>Modo demo activo:</span><span className="portal-chip">Dirección general · CEO</span></div><div className="profile"><span className="avatar"><Icon name="user" size={18}/></span><span>{accountName}</span><Logout/></div></header>
      <main>
        ...
        <footer><span>ENDO WOMAN · CENTRO GINECOLÓGICO</span><span>Dashboard de muestra · Acceso autenticado</span></footer>
      </main>
    </div>
  </div>;
}
